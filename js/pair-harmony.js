...
window.PairHarmony = (function() {
  ...
  function generateCnCnReading(renA, renB, rr, score, rawScore) {
    var parts = [];
    parts.push('【關係本質】' + renA.element + '與' + renB.element + '的組合，彷彿' +
      EL[renA.element].nature + '遇上了' + EL[renB.element].nature + '。');

    if (rr === '相生' || rr === '被生') {
      parts.push('這是一段互相滋養的關係。五行循環中，能量自然地從一方流向另一方，形成美好的給予與接受。' +
        '你們在一起時，會發現很多事情變得輕鬆順利，彷彿有看不見的力量在支持著彼此。');
    } else if (rr === '相同') {
      parts.push('你們像兩棵同一品種的樹，能完全理解彼此的成長節奏。共同的語言與默契是你們的強項，' +
        '但也要記得多引入不同的視角，讓關係持續有新的養分。');
    } else {
      parts.push('五行的衝突不代表關係的失敗，而是提醒你們需要更多的自覺與努力。' +
        '了解彼此的本質差異後，可以刻意練習站在對方的角度思考，化阻力為助力。');
    }

    if (score >= 80) {
      parts.push('整體而言，這是一組極為契合的中文姓名組合。無論是合作、友誼或更親密的關係，都有極佳的先天條件。');
    } else if (score >= 60) {
      parts.push('整體配置良好，雖然不是完美無缺，但只要雙方願意付出，關係有很大的成長空間。');
    } else if (score >= 40) {
      parts.push('這組配對需要較多的磨合與包容。但命理只是參考，真正的關係品質取決於雙方的用心程度。');
    } else {
      parts.push('命理顯示較多衝突點，建議在重要決策上多加謹慎。不過每一段關係都有其獨特價值，關鍵是從中學到什麼。');
    }

    parts.push('【給' + renA.element + '方的話】你的本質是' + EL[renA.element].nature + '。在關係中，善用你的' + EL[renA.element].style + '的優勢。');
    parts.push('【給' + renB.element + '方的話】你的本質是' + EL[renB.element].nature + '。在關係中，記得發揮你' + EL[renB.element].style + '的特質。');
  ...
  function generateEnEnReading(aD, bD, aS, bS, aP, bP, score) {
    var parts = [];
    parts.push('【數字能量】命運' + aD + '與命運' + bD + '的相遇，靈魂' + aS + '與' + bS + '的對話，個性' + aP + '與' + bP + '的互動。');

    if (aD === 1 || bD === 1) parts.push('數字1的能量帶來領導力與開創性，在關係中會自然地想要主導。需注意給對方足夠的空間。');
    if (aD === 2 || bD === 2) parts.push('數字2帶來敏感與和諧的追求，對關係品質有很高的要求，也願意為此付出努力。');
    if (aD === 3 || bD === 3) parts.push('數字3帶來創意與表達的能量，相處中充滿歡笑與新鮮感，是關係中的開心果。');
    if (aD === 5 || bD === 5) parts.push('數字5帶來自由與變化，需注意不要讓對方感到被束縛，同時也要給予足夠的安全感。');
    if (aD === 6 || bD === 6) parts.push('數字6帶來責任與關懷，是典型的照顧者，但也要記得照顧自己。');
    if (aD === 7 || bD === 7) parts.push('數字7帶來深度與內省，重視心靈交流勝於表面互動，需要伴侶的理解與耐心。');
    if (aD === 8 || bD === 8) parts.push('數字8帶來成就與權力，務實且有企圖心，關係中可能較注重實際層面。');
    if (aD === 9 || bD === 9) parts.push('數字9帶來博愛與奉獻，心胸寬大但有時可能忽略身邊最親近的人的需求。');

    if (score >= 80) parts.push('整體而言，這對英文名組合擁有極佳的數字相容性，各方面的能量都互相支持。');
    else if (score >= 60) parts.push('整體數字相容度良好，主要數字間有不錯的互動，部分領域可以更加磨合。');
    else parts.push('數字相容性中等，建議在溝通和互相理解上多下功夫，數字差異可以成為成長的養分。');
